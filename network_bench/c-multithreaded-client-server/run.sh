...
benchmark(){
    ...
    # ./server_poll $port &
    # sleep 5
    # ./client 127.0.0.1 $port $block_size $client_num $req_num
    ...
    cd ../../src/libos/crates/async-socket/
    cargo run --quiet --release --example tcp_echo > /dev/null 2>&1 &
    sleep 5
    cd ../../../../network_bench/c-multithreaded-client-server/
    ./client 127.0.0.1 $port $block_size $client_num $req_num | tee -a "$file_name"
    ...
    cd ../../src/libos/crates/async-socket/examples/sgx
    cd bin && ./app &
    sleep 5
    cd ../../../../../../network_bench/c-multithreaded-client-server/
    ./client 127.0.0.1 $port $block_size $client_num $req_num  | tee -a "$file_name"
